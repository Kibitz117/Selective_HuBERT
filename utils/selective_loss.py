...
#         # compute penalty (=psi)
#         coverage = torch.tensor([self.coverage], dtype=torch.float32,  device=self.device)
#         penalty = torch.max(coverage - empirical_coverage, 
#                             torch.tensor([0.0], dtype=torch.float32, device=self.device)) ** 2
#         penalty *= self.lm
...
#         ce_loss = self.cross_entropy(auxiliary_out, target)
...
class SelectiveLoss(torch.nn.Module):
    ...
    def forward(self, prediction_out, selection_out, auxiliary_out, target, threshold=0.5, mode='train'):
        ...
        penalty = torch.max(coverage-empirical_coverage, torch.tensor([0.0], dtype=torch.float32, requires_grad=True, device=self.device))**2
        penalty *= self.lm
    ...
    def get_selective_loss(self, prediction_out, selection_out, target):
        ...
        empirical_risk_variant = torch.mean(ce * selection_out.view(-1))
        empirical_coverage = selection_out.mean() 
        penalty = torch.max(self.coverage - empirical_coverage, torch.tensor([0.0], dtype=torch.float32, requires_grad=True, device=self.device))**2
        loss = empirical_risk_variant + self.lm * penalty
        return loss
